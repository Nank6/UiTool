<template>
  <div class="sys-page">
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" :max-height="maxHeight" border fit highlight-current-row>
      ...
      <el-table-column label="标准报文" width="400" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.requestStandard }}</span>
        </template>
      </el-table-column>

      <el-table-column label="抓取报文" width="400" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.requestResult }}</span>
        </template>
      </el-table-column>

      <el-table-column label="不同" width="400" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.requestDiff }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      request({
        url: '/ComparisonController/getComparisonList',
        method: 'post',
        data: {
    ...
    deleteall() {
      request({
        url: '/ComparisonController/deleteAll',
        method: 'post',
        data: {}
      }).then(() => {
        this.getList()
